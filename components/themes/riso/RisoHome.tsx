...
export function RisoHome() {
  ...
  return (
    <div style={{ background: '#fffde7', minHeight: '100vh', fontFamily: "'Noto Sans TC', sans-serif", position: 'relative', overflow: 'hidden' }}>
      ...
      {/* Scattered county map — fixed background */}
      <ScatteredTaiwanMap
        selected={county}
      ...
      {/* Filter bar */}
      <div style={{ position: 'relative', zIndex: 10, padding: '0.5rem 2rem', display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        {tab === 'search' && (
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder="關鍵字…" autoFocus
            style={{ border: '3px solid #0066cc', background: 'rgba(255,253,231,0.9)', padding: '6px 12px', fontFamily: "'Noto Sans TC',sans-serif", fontSize: '0.9rem', outline: 'none', transform: 'rotate(-0.5deg)' }} />
        )}

        {tab === 'map' && county && (
          <div style={{ border: '2px solid #e65100', padding: '0.25rem 0.7rem', transform: 'rotate(-0.8deg)', display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'rgba(255,253,231,0.9)' }}>
            <span style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: '1rem', letterSpacing: '0.1em', color: '#e65100' }}>{county}</span>
            <button onClick={() => setCounty(null)} style={{ background: 'none', border: 'none', color: '#e65100', cursor: 'pointer', fontWeight: 700, fontSize: '1rem', lineHeight: 1 }}>×</button>
          </div>
        )}

        {tab === 'map' && !county && (
          <span style={{ fontFamily: "'Noto Sans TC',sans-serif", fontSize: '0.7rem', color: '#e65100', opacity: 0.6, transform: 'rotate(-0.5deg)', display: 'inline-block' }}>← 點選背景縣市形狀篩選</span>
        )}
      </div>

      {/* Expedition cards — semi-transparent so counties show through */}
      <div style={{ position: 'relative', zIndex: 10, padding: '0.5rem 2rem 6rem', display: 'flex', flexDirection: 'column', gap: 0, maxHeight: 'calc(100vh - 120px)', overflowY: 'auto' }}>
